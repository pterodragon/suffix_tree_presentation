...
\node[yshift=-30pt](bottom)at (current page.north) {  } child { node[draw=red,double](q) {  } child { node[draw=red,double](qc) {  } edge from parent node[rectangle,draw=green,fill=white] { $c$ }   }edge from parent node[rectangle,draw=green,fill=white] { $\Sigma$ }   } ;\draw[style=sll,draw opacity=0.5](q) to (bottom);
