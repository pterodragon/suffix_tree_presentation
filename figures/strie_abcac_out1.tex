...
\node[yshift=-30pt](bottom)at (current page.north) {  } child { node[draw=red](q) {  } child { node[draw=red](qa) {  } edge from parent node[rectangle,draw=green,fill=white] { $a$ }   }edge from parent node[rectangle,draw=green,fill=white] { $\Sigma$ }   } ;\draw[style=sll,draw opacity=0.5](q) to (bottom);
